#!/usr/bin/env python3
"""Bootstrap v4A orchestrator (prior-errors only).

No pattern injection during rounds.
Learning signal is a rolling window of prior errors.
Default policy: R1-R3 no prior-errors, R4+ prior-errors from last 3 rounds.
"""
...
def build_prompt(batch: List[dict], feature_spec: List[dict], prior_errors: List[Dict[str, Any]] = None) -> str:
...
def main() -> None:
    ...
    for r in range(args.start_round, args.rounds + 1):
        ...
        for bi, bkey in enumerate(batch_ids, start=1):
            ...
            prompt = build_prompt(batch, feature_spec, prior_errors if prior_errors else None)
