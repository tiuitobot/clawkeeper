#!/usr/bin/env python3
"""Bootstrap v2 orchestrator.

R1-R3 baseline (no learned patterns), R4+ with accumulated abstract patterns.
...
def build_prompt(batch: List[dict], feature_spec: List[dict], patterns: List[str]) -> str:
...
def main() -> None:
    ...
    for r in range(args.start_round, args.rounds + 1):
        ...
        if r >= 4:
            for rr in range(1, r):
                pf = OUT / f"round_{rr}_patterns.json"
                if pf.exists():
                    patterns.extend([p["pattern"] for p in json.load(pf.open()).get("patterns", []) if p.get("pattern")])
        ...
        run_py(
            ...
            [
                ...
                str(errors_path),
                "--output",
                str(OUT / f"round_{r}_patterns.json"),
            ]
